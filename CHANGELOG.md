...
- **Extended Background Line Illumination in Spicy Lyrics**: Fixed an issue where lead lines and standalone background lines remained frozen in the illuminated `.Active` state past their vocal duration when saved/exported TTML files were played in Spicy Lyrics.
  - In `ttml-writer`: Standalone background lines now export into their own independent `<p begin="..." end="...">` elements so each background vocal maintains its own isolated start and end times rather than inheriting an extended `<p end="...">` covering subsequent backgrounds.
  - In `ttml-parser`: `parseLyric` now derives line `startTime` and `endTime` directly from word-level token timestamps (`line.words`) when available, preventing lead lines from inheriting the container's extended `end` attribute from child `<span ttm:role="x-bg">` spans.
